...
| `Cov(AP_i, AP_j)` for a shared positive | **ASSUMED model, then exact** | Nothing in Lean covers it. Given the model it is exact, checked against exact rational integration over the shared quantile for all `1 <= M <= L <= 7`, and against direct simulation of the model. |
