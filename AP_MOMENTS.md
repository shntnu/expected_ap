...
| `E[AP^3]` closed form (`eap3`) | **VERIFIED numerically** | Two independent derivations (exact ansatz identification, mechanical 6-index pattern expansion) converged on the identical formula; exact against enumeration for all `1 <= M <= L <= 13` and a moment DP to `L = 120`. Needs the Euler-sum generator `G_L = sum_{k<=L} H_k/k^2`; the pure weight-3 harmonic basis admits no representation with polynomial coefficients up to degree 12 over the natural denominators (certified modularly; full irreducibility conjectured). Not formalised in Lean. |
...
**6. Model the Gram coupling.** The hardest item, listed last because it is the only one without a mapped path - but it is the binding constraint for real data at large `n`, and everything above inherits it. All tier-2 results assume each profile pair shares exactly one similarity value; in real cosine retrieval every similarity is an inner product over the *same* feature draws, and that Gram structure makes the true null 2 to 3 percent wider in sd and up to about 4x more right-skewed at `n = 50` than the tier-2 model predicts (the calibration section has the measurements). The encouraging fact from the calibration is that a three-moment null matched to the *measured* Gram moments reaches nominal at both alpha levels, so no new tail machinery is needed - what is missing is a model of the Gram null's moments. The concrete attack: replace the tier-2 generative story with similarities `s_ij = <f_i, f_j>` for iid feature vectors, where every pairwise similarity involving profile `i` shares the vector `f_i`, and derive the covariance and third-moment corrections this induces beyond the shared-pair term (plausibly as a `1/n_feat` expansion, since the measured gap shrinks as features grow). Until that exists, the honest advice in the limitations section stands: permute for deep-tail inference on real data at large `n`.
