...
\small\{\href{mailto:shantanu@broadinstitute.org}{shantanu},\href{mailto:anne@broadinstitute.org}{anne}\}@broadinstitute.org
\end{center}}
\makeatother
...
\maketitle
\begin{abstract}
The mean and variance of full-list, non-interpolated average precision under a uniformly random ranking depend on both the fixed number of relevant items and the list length.
...
For list length $L$, the variance is asymptotic to $p(1-p)/L$ at fixed prevalence $p\in(0,1)$ and to $\pi^2/(6ML)$ at fixed positive count $M\ge1$.
...
Our harmonic expectation formula and its exchangeability derivation were obtained independently of Manzhos et al. and recorded in the repository on September 7, 2025~\cite{early}, before their preprint of November 4, 2025~\cite{manzhos}.
The Lean expectation proof followed on March 23, 2026, and the second-moment derivation and variance verification on July 23, 2026.
The accompanying verification record links these stages to their source commits.
...
Rank 1 is the highest-priority item, and prevalence is the relevant fraction $M/L$.
...
\end{equation}
With tied scores, equation~\eqref{eq:ap} uses an ordering of individual items; evaluating an entire tied group at a common score threshold defines a different statistic.
Any tie-breaking procedure is assumed to preserve the uniform null law on rank subsets.
...
The mean is $3403/7840\approx0.434056$, compared with prevalence $0.25$, and the standard deviation is approximately $0.208750$.
Thus a score near $0.43$ is close to the chance mean for this list.

The exact formulas quantify finite-list effects that the leading asymptotic terms can miss.
For example, at $L=100$ and $M=10$, the exact variance is approximately $0.002876$, while $p(1-p)/L=0.0009$.
Use the exact moments for a given list and the asymptotic expressions to describe limiting behavior.
The one-standard-deviation band in panel C summarizes spread around the null mean; its probability coverage depends on the discrete AP law.
...
These are standard logical foundations used by Mathlib, and the audit checks that the results have no admitted-proof dependencies.
...
The Python demonstration checks the mean and variance by exhaustive enumeration for every $1\le L\le10$ and $0\le M\le L$, including the zero-positive convention.
...
The supplement is supplied as \texttt{anc/supplement.zip} with the arXiv source and is available in the \href{https://github.com/shntnu/expected_ap}{paper repository}.
For the arXiv source download, unpack that supplement alongside \texttt{main.tex} and run \texttt{bash reproduce.sh} from the source root to generate the paper, figure, tables, and verification record.
The Python demonstration also runs separately as \texttt{uv run -{}-locked python demo.py}.
...
\bibitem{early} S. Singh.
Expected Average Precision Under Random Ranking: A Finite-Sample Analysis.
Repository manuscript, September 7, 2025, Theorem 1 and proof.
Commit \texttt{4efbcf5}; \href{https://github.com/shntnu/expected_ap/blob/4efbcf57d0d28a158dcbbe52872847d8e21ba130/expected_ap.md}{archived source}.
\end{thebibliography}
\end{document}
